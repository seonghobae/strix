...
def test_recover_container_state(mock_docker_client):
    container = MagicMock()
    container.attrs = {
        "Config": {"Env": ["SOME_VAR=1", "TOOL_SERVER_TOKEN=secret_token"]},
        "NetworkSettings": {
            "Ports": {
    ...
    assert runtime._tool_server_token == "secret_token"
...
@patch("strix.runtime.docker_runtime.DockerRuntime._wait_for_tool_server")
@patch("strix.runtime.docker_runtime.time.sleep")
def test_create_container_failure_retry(mock_sleep, mock_wait, mock_verify, mock_config, mock_docker_client):
    mock_config.side_effect = lambda k: "test-image" if k == "strix_image" else None
...
def test_copy_local_directory_to_container(mock_rglob, mock_is_dir, mock_exists, mock_docker_client):
...
async def test_create_sandbox_success(mock_register, mock_docker_client):
    ...
    runtime._tool_server_token = "token123"
    
    with patch.object(runtime, "_get_scan_id", return_value="scan-123"):
...
async def test_create_sandbox_missing_token(mock_docker_client):
    ...
    with patch.object(runtime, "_get_scan_id", return_value="scan-123"):
        with patch.object(runtime, "_get_or_create_container", return_value=container):
            with pytest.raises(RuntimeError, match="Tool server not initialized"):
...
async def test_create_sandbox_with_local_sources(mock_register, mock_docker_client):
    ...
    with patch.object(runtime, "_get_scan_id", return_value="scan-123"):
        with patch.object(runtime, "_get_or_create_container", return_value=container):
            with patch.object(runtime, "_copy_local_directory_to_container") as mock_copy:
                with patch.object(runtime, "_resolve_docker_host", return_value="localhost"):
...
async def test_create_sandbox_container_id_none(mock_docker_client):
    ...
    with patch.object(runtime, "_get_scan_id", return_value="scan-123"):
        with patch.object(runtime, "_get_or_create_container", return_value=container):
            with pytest.raises(RuntimeError, match="Docker container ID is unexpectedly None"):
                await runtime.create_sandbox("agent-123")
