...
def _validate_root_agent(agent_state: Any) -> dict[str, Any] | None:
    """Validate that the current agent is the root agent.

    Args:
        agent_state: The state of the current agent.

    Returns:
        A dictionary containing an error message if the agent is not the root agent,
...
def _check_active_agents(agent_state: Any = None) -> dict[str, Any] | None:
    """Check if there are any active agents before finishing the scan.

    Args:
        agent_state: The state of the current agent.

    Returns:
        A dictionary containing an error message if there are active agents,
        or None if there are no active agents.
    """
    try:
...
def finish_scan(
    ...
    """Finish the current scan and record findings.

    Args:
        executive_summary: High-level summary of the findings.
        methodology: Explanation of how the scan was performed.
        technical_analysis: Detailed technical breakdown of findings.
        recommendations: Actionable advice to address findings.
        agent_state: The state of the current agent.

    Returns:
        A dictionary indicating the result of the operation, including success
        status and vulnerability count.
    """
